...
private def findPrefixTranslation? (n : Name) (t : TranslateData) :
    CoreM (Option TranslationInfo) := do
  let env ← getEnv
...
private def etaExpandN (n : Nat) (e : Expr) : MetaM Expr := do
...
private def updateAndAddDecl (t : TranslateData) (tgt : Name) (srcDecl : ConstantInfo)
...
private def declUnfoldSimpAuxLemmas (decl : ConstantInfo) : MetaM ConstantInfo := do
...
private def findTargetName (env : Environment) (t : TranslateData) (src rootSrc rootTgt : Name) :
    CoreM Name := do
...
-/
private def findAuxDecls (decl : ConstantInfo) (pre : Name) : CoreM (Array Name) := do
...
private def copyInstanceAttribute (src tgt : Name) : CoreM Unit := do
...
private def warnAttr {α β : Type} [Inhabited β] (stx : Syntax) (attr : SimpleScopedEnvExtension α β)
...
private def warnParametricAttr {β : Type} [Inhabited β] (stx : Syntax)
    (attr : ParametricAttribute β) (thisAttr attrName src tgt : Name) : CoreM Unit :=
...
private def translateLemmas {m : Type → Type} [Monad m] [MonadError m] [MonadLiftT CoreM m]
...
private def targetName (t : TranslateData) (cfg : Config) (src : Name) : CoreM Name := do
  if cfg.self then
...
private partial def checkExistingType (t : TranslateData) (src tgt : Name) (cfg : Config) :
...
private def proceedFields (t : TranslateData) (src tgt : Name) (reorder : Reorder)
