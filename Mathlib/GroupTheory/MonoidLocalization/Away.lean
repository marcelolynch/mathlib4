...
private noncomputable def AwayMap.invSelf : N := F.mk' 1 ⟨x, mem_powers _⟩
...
private noncomputable def awayToAwayRight (y : M) (G : AwayMap (x * y) P) : N →* P :=
  F.lift x <|
...
private noncomputable def AwayMap.negSelf : B :=
...
private noncomputable def awayToAwayRight (y : A) (G : AwayMap (x + y) C) : B →+ C :=
  F.lift x <|
