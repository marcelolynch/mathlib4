...
private theorem treeHom_eq {a : G} (p : Path (root T) a) : treeHom T a = homOfPath T p := by
...
private theorem loopOfHom_eq_id {a b : Generators G} (e) (H : e ∈ wideSubquiverSymmetrify T a b) :
...
private lemma endIsFree : IsFreeGroup (End (root' T)) :=
...
private theorem path_nonempty_of_hom {G} [Groupoid.{u, u} G] [IsFreeGroupoid G] {a b : G} :
