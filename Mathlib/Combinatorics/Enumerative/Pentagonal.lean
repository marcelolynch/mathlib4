...
private theorem pentagonal_def (k : ℤ) : pentagonal k = (k * (3 * k - 1) / 2).toNat := by rfl

private theorem pentagonal_neg (k : ℤ) : pentagonal (-k) = (k * (3 * k + 1) / 2).toNat := by
  grind [pentagonal_def]

private theorem natCast_pentagonal (k : ℤ) : (pentagonal k : ℤ) = k * (3 * k - 1) / 2 := by
  rcases k with (_ | _) | _ <;> grind [pentagonal_def]

private theorem two_mul_natCast_pentagonal (k : ℤ) : 2 * (pentagonal k : ℤ) = k * (3 * k - 1) := by
  rw [natCast_pentagonal]
  exact Int.two_mul_ediv_two_of_even (by grind)

private theorem two_mul_natCast_pentagonal_neg (k : ℤ) : 2 * (pentagonal (-k) : ℤ) = k * (3 * k + 1) := by
  grind [two_mul_natCast_pentagonal]

private theorem pentagonal_injective : Function.Injective pentagonal := by
  intro x y h
...
private theorem pentagonal_lt_pentagonal_neg {k : ℤ} (h : 0 < k) : pentagonal k < pentagonal (-k) := by
  grind [natCast_pentagonal]

private theorem pentagonal_neg_lt_pentagonal_add_one {k : ℤ} (h : 0 ≤ k) :
    pentagonal (-k) < pentagonal (k + 1) := by
  grind [natCast_pentagonal]

private theorem pentagonal_strictMonoOn : StrictMonoOn pentagonal (Set.Ici 0) := by
  apply strictMonoOn_of_lt_add_one Set.ordConnected_Ici
  grind [natCast_pentagonal]

private theorem pentagonal_strictAntiOn : StrictAntiOn pentagonal (Set.Iic 0) := by
