...
private def next [RandomGen g] [Monad m] : RandGT g m Nat := do
  let rng := (← get).down
...
private def split {g : Type} [RandomGen g] [Monad m] : RandGT g m g := do
  let rng := (← get).down
...
private def range {g : Type} [RandomGen g] [Monad m] : RandGT g m (Nat × Nat) := do
  let rng := (← get).down
...
-/
private def runRand (cmd : RandT m α) : m α := do
...
private def runRandWith (seed : Nat) (cmd : RandT m α) : m α := do
